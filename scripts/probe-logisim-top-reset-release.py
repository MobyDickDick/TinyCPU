#!/usr/bin/env python3
"""Build a temporary 8/8 TinyCPUMain project for the AP 20.5 reset probe."""
...
def build_probe(source: Path, destination: Path) -> None:
    ...
    for component in circuit.findall("comp"):
        ...
        if label not in {
            "CLK", "RESET", "HALTED", "MONITOR_PC_OUT"
        }:
            continue
        ...
        if label == "CLK":
            ...
            reset_clock = ET.SubElement(
                circuit, "comp", {"lib": "0", "loc": "(290,440)", "name": "Clock"}
            )
            add_attribute(reset_clock, "highDuration", "100")
            add_attribute(reset_clock, "lowDuration", "2")
            ET.SubElement(circuit, "wire", {"from": "(290,440)", "to": "(310,440)"})
        elif label == "HALTED":
            next(
    ...
    # Keep the diagnostic table deliberately small.  These top-level output
    # pins are only observation sinks in this temporary start circuit, so
    # replacing unrelated ones with probes does not change the CPU nets.
    for component in circuit.findall("comp"):
        values = attributes(component)
        if (
    ...
    # Observe the same reset net both at its source and exactly where it enters
    # FetchDecode.  Separate pins make accidental divergence visible in the
    # change-driven table without changing the checked-in project.
    add_probe(circuit, "(330,450)", "RESET_SOURCE_PROBE", "(330,440)")
    add_probe(circuit, "(870,450)", "RESET_FETCH_PROBE", "(870,440)")
