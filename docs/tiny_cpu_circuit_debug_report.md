...
| 20.8 Endabnahme und Funktionsstatus aktualisieren | in Bearbeitung | Der zusammengeführte Kandidat `29e10e5` besteht Offline- und elektrisches Gate in einem frischen Checkout zweimal. Da weder `DISPLAY` noch `Xvfb`, `Xorg` oder `xdotool` verfügbar sind und die Paketquelle `xvfb` und `xdotool` nicht enthält, bleibt ausschließlich der extern auszuführende GUI-Kurztest offen. |
