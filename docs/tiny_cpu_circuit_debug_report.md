...
### Abschluss von AP 20.6: vollständige ISA- und Fehlerregression

- **Ausgangs-Commit:** `22c8af49d232612ae22f5329adf2b35fbd68609e`.
- **Arbeitsbaum vor der Abnahme:** sauber (`git status --porcelain=v1`
  lieferte keine Ausgabe).
- **Simulator:** Logisim-evolution 4.1.0 aus
  `.venv/Include/logisim-evolution-4.1.0-all.jar`.

Nach der semantischen Korrektur der beiden `JumpBox`-Topologieprüfungen
besteht das Offline-Gate wieder vollständig. Die anschließende elektrische
Profilabnahme führt zwei unabhängige Minimalkernläufe und alle 61 Matrixfälle
aus. Die Matrix umfasst alle 50 Opcode-Positivfälle, die jeweils zusätzlichen
nicht genommenen Pfade der fünf bedingten Sprünge sowie alle sechs
Sticky-Fehlerfälle. Kein Fall wird übersprungen oder nur statisch bewertet.
...
scripts/test-offline.sh
LOGISIM_JAR="$PWD/.venv/Include/logisim-evolution-4.1.0-all.jar" \
  LOGISIM_OUTPUT="$PWD/artifacts/ap20.6-final" scripts/test-logisim.sh
```

| Lauf | Exitcode | Ergebnis |
|---|---:|---|
| Offline-Gate | 0 | Verifier, statischer Schaltungscheck und alle 86 Unit-Tests bestehen. |
| Elektrische 16/12-Profilabnahme | 0 | Beide Kernläufe und alle 61 Matrixfälle bestehen gegen das Python-Referenzmodell. |

Die unveränderten elektrischen Tabellen liegen gemäß Artefaktrichtlinie
unter `artifacts/ap20.6-final/` und werden nicht eingecheckt. AP 20.6 ist damit
...
### AP 20.8: wiederholte automatische Endabnahme

- **Kandidat:** `955347029ea77a7b1735b65242978616768a87c7`
...
Im frischen Worktree wurden beide vorgeschriebenen Gates zweimal nacheinander
ausgeführt. Beide Offline-Läufe validierten 10 JSON-Dateien, 23
Logisim-Projekte mit 46 Schaltungen und 2.679 rechtwinkligen Leitungen sowie
alle 87 Unit-Tests. Beide elektrischen Läufe bestanden jeweils mit zwei
identischen Minimalkerntraces und allen 61 Matrix-Fixtures: 50 positive
Opcode-Fälle, fünf zusätzliche nicht genommene Sprungpfade und sechs
Sticky-Fehlerfälle. Der frische Worktree blieb nach jedem Lauf sauber.
...
|---|---:|---:|---|
| Offline 1 | 0 | 13 s | Verifier, Schaltungscheck und 87 Tests bestanden |
| Elektrisch 1 | 0 | 125 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Offline 2 | 0 | 12 s | Verifier, Schaltungscheck und 87 Tests bestanden |
| Elektrisch 2 | 0 | 126 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Der bislang nur implizit referenzierte GUI-Ablauf steht nun als konkrete
Schrittfolge in `hardware/logisim/README.md`. Die Ausführungsumgebung dieses
Laufs besitzt jedoch weder `DISPLAY` noch einen virtuellen X-Server; die
Nachinstallation von `xvfb` und `xdotool` scheiterte an der gesperrten
Paketquelle. Eine TTY-Simulation wird nicht als manueller GUI-Nachweis
umetikettiert. Deshalb bleiben AP 20.8 und die endgültige Statusfreigabe offen,
...
#### Endabnahme nach Ergänzung der GUI-Starthinweise

- **Kandidat:** `b3cdc9b9c66628552e7ebefcc130dda00c1b26b6`
- **Datum:** 19. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
Nach der Ergänzung der plattformspezifischen Hinweise zum manuellen GUI-Start
wurden beide automatischen Gates auf dem aktuellen Kandidaten erneut zweimal
nacheinander ausgeführt. Die Offline-Läufe bestanden mit allen 104 Unit-Tests;
die elektrischen Läufe bestanden mit je zwei identischen Minimalkerntraces und
allen 61 Matrix-Fixtures. Der frische Worktree blieb nach allen vier Läufen
sauber.
...
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT="$PWD/artifacts/ap20.8-b3cdc9b9/electrical-1" \
  scripts/test-logisim.sh
scripts/test-offline.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT="$PWD/artifacts/ap20.8-b3cdc9b9/electrical-2" \
  scripts/test-logisim.sh
```
...
|---|---:|---:|---|
| Offline 1 | 0 | 13 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 131 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Offline 2 | 0 | 14 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 2 | 0 | 134 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Die Umgebung besitzt weiterhin weder ein gesetztes `DISPLAY` noch `Xvfb`,
`Xorg` oder `xdotool`. Gemäß der Paketgrenze wurde die erfolgreiche elektrische
Simulation nicht als manueller Sichtnachweis ausgegeben. AP 20.8 bleibt daher
nur bis zum externen GUI-Kurztest offen; die automatische Endabnahme ist für
den aktuellen Kandidaten vollständig grün.

#### Endabnahme des aktuellen Hauptzweigstands

- **Kandidat:** `9bb2ce9f52171ae422f5cb088d899f1a655464c1`
- **Datum:** 19. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
Der nach den zuletzt zusammengeführten Dokumentationsänderungen aktuelle
Hauptzweigstand wurde erneut mit beiden vorgeschriebenen Gates zweimal
nacheinander abgenommen. Beide Offline-Läufe validierten elf JSON-Dateien, 23
Logisim-Projekte mit 46 Schaltungen und 2.764 rechtwinkligen Leitungen, 50
Opcodes, sechs Sticky-Error-Fixtures und alle 104 Unit-Tests. Die elektrischen
Läufe bestanden mit je zwei identischen Minimalkerntraces und allen 61
Matrix-Fixtures. Der frische Worktree blieb nach jedem Lauf unverändert.

```bash
scripts/test-offline.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT="$PWD/artifacts/ap20.8-9bb2ce9/electrical-1" \
  scripts/test-logisim.sh
scripts/test-offline.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT="$PWD/artifacts/ap20.8-9bb2ce9/electrical-2" \
  scripts/test-logisim.sh
```
...
|---|---:|---:|---|
| Offline 1 | 0 | 13 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 124 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Offline 2 | 0 | 13 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 2 | 0 | 123 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Der GUI-Preflight ergab weiterhin ein leeres `DISPLAY`; außerdem sind weder
`Xvfb` noch `Xorg` oder `xdotool` installiert. Damit kann diese Umgebung keinen
sichtbaren, manuell bedienten Logisim-Desktop bereitstellen. Insbesondere wird
der erfolgreiche Tabellenlogger-Lauf nicht als GUI-Kurztest umetikettiert.
Reset, Einzeltakt, die sichtbaren Ausgaben `3, 2, 1`, Normalhalt und Fehlerhalt
müssen daher weiterhin extern nach der dokumentierten Bedienfolge protokolliert
...
#### Endabnahme des aktuellen Arbeitsstands

- **Kandidat:** `090183ef04c0b6806f227ce0a45a789096fe460f`
...
Der aktuelle Stand nach dem Zusammenführen der vorherigen Abnahme wurde erneut
in einem frischen, abgetrennten Worktree geprüft. Beide Offline-Läufe bestanden
mit allen 104 Unit-Tests. Die beiden elektrischen Läufe bestanden mit je zwei
identischen Minimalkerntraces und sämtlichen 61 Matrix-Fixtures. Der
Prüf-Worktree blieb nach allen vier Läufen unverändert.

```bash
scripts/test-offline.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-090183e/electrical-1 \
  scripts/test-logisim.sh
scripts/test-offline.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-090183e/electrical-2 \
  scripts/test-logisim.sh
```
...
|---|---:|---:|---|
| Offline 1 | 0 | 12 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 105 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Offline 2 | 0 | 12 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 2 | 0 | 105 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Der GUI-Preflight lieferte weiterhin ein leeres `DISPLAY`; `Xvfb`, `Xorg` und
`xdotool` fehlen ebenfalls. Damit kann diese Umgebung keinen sichtbaren,
manuell bedienten Logisim-Desktop bereitstellen. Der erfolgreiche
Tabellenlogger-Lauf wird nicht als GUI-Kurztest gewertet. Reset, Einzeltakt,
die sichtbaren Ausgaben `3, 2, 1`, Normalhalt und Fehlerhalt müssen weiterhin
extern anhand der dokumentierten Bedienfolge protokolliert werden. AP 20.8 und
die endgültige Statusfreigabe bleiben ausschließlich bis zu diesem Sichtnachweis
offen; die automatische Endabnahme des aktuellen Kandidaten ist zweimal
reproduzierbar grün.

#### Endabnahme nach Zusammenführung der letzten Kandidatenprüfung

- **Kandidat:** `c13ab89cd3b698323d38d33f75982741b28e265e`
- **Datum:** 20. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
Der zusammengeführte Stand der vorherigen AP-20.8-Abnahme wurde in einem
frischen, abgetrennten Worktree erneut geprüft. Beide Offline-Läufe bestanden
mit allen 104 Unit-Tests. Zwei aufeinanderfolgende elektrische Läufe bestanden
mit je zwei identischen Minimalkerntraces und sämtlichen 61 Matrix-Fixtures.
Der Prüf-Worktree blieb nach allen Läufen unverändert.
...
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-c13ab89/electrical-2 \
  scripts/test-logisim.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-c13ab89/electrical-3 \
  scripts/test-logisim.sh
```
...
| Offline 2 | 0 | 13 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 124 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Elektrisch 2 | 0 | 121 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Der GUI-Preflight lieferte ein leeres `DISPLAY`; `Xvfb`, `Xorg` und `xdotool`
sind weiterhin nicht installiert. Ein sichtbarer und manuell bedienbarer
...
- **Kandidat:** `c3fbbe2d68fdae646f5450d98727fa5ba0bbe1ff`
- **Datum:** 20. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-c3fbbe2/electrical-1 \
  scripts/test-logisim.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-c3fbbe2/electrical-2 \
  scripts/test-logisim.sh
```

| Lauf | Exitcode | Laufzeit | Ergebnis |
|---|---:|---:|---|
| Offline 1 | 0 | 8 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Offline 2 | 0 | 8 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 89 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Elektrisch 2 | 0 | 90 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
...
weiterhin extern anhand der dokumentierten Bedienfolge protokolliert werden.
AP 20.8 und die endgültige Statusfreigabe bleiben ausschließlich bis zu diesem
Sichtnachweis offen; die automatische Endabnahme des aktuellen Merge-Stands
ist zweimal reproduzierbar grün.
...
`E` unbekannte beziehungsweise elektrische Fehlerwerte; sie belegen weder den
...
- **Kandidat:** `367ecf45f88541af2cccf9f316a34f4551a33368`
- **Datum:** 20. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
Der Stand einschließlich des getrennten Operator-Panel-Vorschlags wurde in
einem frischen, abgetrennten Worktree erneut vollständig automatisch
abgenommen. Beide Offline-Läufe bestanden mit Verifier, Schaltungscheck und
allen 104 Unit-Tests. Beide elektrischen Läufe bestanden mit je zwei
identischen Minimalkerntraces und sämtlichen 61 Matrix-Fixtures. Der
Prüf-Worktree blieb unverändert. Die Roh- und Normaltraces liegen außerhalb
des Repositorys unter `/tmp/ap20.8-367ecf4/`.
...
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-367ecf4/electrical-1 \
  scripts/test-logisim.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-367ecf4/electrical-2 \
  scripts/test-logisim.sh
```

| Lauf | Exitcode | Ergebnis |
|---|---:|---|
| Offline 1 | 0 | Verifier, Schaltungscheck und 104 Tests bestanden |
| Offline 2 | 0 | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Elektrisch 2 | 0 | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |

Der GUI-Preflight fand weiterhin kein `DISPLAY`; außerdem fehlen `Xvfb`,
`fluxbox`, `x11vnc`, `xdpyinfo`, `xauth` und `xdotool`. Damit steht in dieser
nicht-interaktiven Umgebung keine sichtbare und manuell bedienbare Oberfläche
zur Verfügung. Ein virtueller, unbeobachteter Desktop oder der erfolgreiche
Tabellenlogger-Lauf wird ausdrücklich nicht als GUI-Kurztest umetikettiert.
AP 20.8 bleibt ausschließlich bis zu einem extern protokollierten Sichtnachweis
für Reset, Einzeltakt, die Ausgaben `3, 2, 1`, Normalhalt und Fehlerhalt offen;
die automatische Endabnahme des aktuellen Kandidaten ist zweimal
aussagekräftig und reproduzierbar grün.

#### Endabnahme des aktuellen Hauptzweigstands

- **Kandidat:** `4f1fec1f99e919c899d575a7c4813d5243fee6c3`
- **Datum:** 20. September 2026
- **Ausführung:** abgetrennter frischer Git-Worktree ohne lokale Änderungen
- **Python:** 3.14.4
- **Java:** OpenJDK 25.0.2 (`25.0.2+10-69`)
...
Der nach der letzten Annahme zusammengeführte Hauptzweigstand wurde in einem
frischen, abgetrennten Worktree erneut automatisch abgenommen. Beide
...
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-4f1fec1/electrical-1 \
  scripts/test-logisim.sh
LOGISIM_JOBS=4 \
  LOGISIM_JAR=/workspace/TinyCPU/.venv/Include/logisim-evolution-4.1.0-all.jar \
  LOGISIM_OUTPUT=/tmp/ap20.8-4f1fec1/electrical-2 \
  scripts/test-logisim.sh
```
...
| Offline 2 | 0 | 12 s | Verifier, Schaltungscheck und 104 Tests bestanden |
| Elektrisch 1 | 0 | 114 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
| Elektrisch 2 | 0 | 263 s | zwei Kernläufe und 61/61 Matrix-Fixtures bestanden |
