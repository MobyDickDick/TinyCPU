...
| 20.8 Endabnahme und Funktionsstatus aktualisieren | in Bearbeitung | Der aktuelle Kandidat `c13ab89` besteht Offline- und elektrisches Gate in einem frischen Checkout zweimal. Da weder `DISPLAY` noch `Xvfb`, `Xorg` oder `xdotool` verfügbar sind, bleibt ausschließlich der extern auszuführende GUI-Kurztest offen. |
